...
import { AlertCircle, CheckCircle2, FileSpreadsheet, X } from 'lucide-vue-next';
...
<template>
    ...
    >
        <div v-if="isVisible && result" class="fixed top-4 right-4 z-50 w-96 max-w-sm">
            <Alert :variant="result.success ? 'default' : 'destructive'" class="relative">
                <CheckCircle2 v-if="result.success" class="h-4 w-4" />
                <AlertCircle v-else class="h-4 w-4" />

                <Button variant="ghost" size="sm" class="absolute top-2 right-2 h-auto w-auto p-1" @click="closeNotification">
                    <X class="h-3 w-3" />
                </Button>
                ...
                <AlertDescription class="pr-6">
                    <div class="space-y-2">
                        ...
                        <!-- Error stats -->
                        <div v-if="!result.success && result.failed_count" class="text-sm">{{ result.failed_count }} registros fallaron</div>
                        ...
                        <div v-if="result.errors && result.errors.length > 0" class="space-y-1">
                            ...
                            <div class="space-y-1">
                                <div
                                    ...
                                    class="rounded bg-red-50 p-1 text-xs"
                                >
