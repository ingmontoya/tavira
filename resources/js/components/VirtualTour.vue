<template>
  <div v-if="isActive" class="virtual-tour">
    ...
    <div 
      ...
      class="fixed bg-black/60 backdrop-blur-sm transition-all duration-300 z-30" 
    ...
    <div 
      v-if="currentStep" 
      class="fixed z-50 bg-white rounded-lg shadow-2xl border border-gray-200 animate-in fade-in duration-300"
      :class="tooltipClasses"
      :style="stepPosition"
    >
      <div class="p-5">
        <div class="flex items-start justify-between mb-3">
          <h3 class="font-semibold text-gray-900 text-base pr-2">{{ currentStep.title }}</h3>
          <button @click="pauseTour" class="text-gray-400 hover:text-gray-600 flex-shrink-0" title="Pausar tour">
            <Icon name="x" class="w-5 h-5" />
          </button>
        </div>
        
        <p class="text-sm text-gray-600 mb-5 leading-relaxed">{{ currentStep.description }}</p>
      ...
      <div 
        class="absolute w-3 h-3 bg-white transform rotate-45 border-l border-t"
        :style="arrowPosition"
      />
    </div>
    ...
    <div 
      ...
      class="fixed border-2 border-blue-500 rounded-lg pointer-events-none animate-pulse shadow-lg z-40"
...
const isCurrentStepInSidebar = computed(() => {
  ...
  const sidebarSteps = ['step-1-programs', 'step-1-periods', 'step-1-study-plans', 'step-1-subjects', 'step-1-groups', 'step-2-students', 'step-3-enrollments', 'step-4-dashboard']
...
const isNavigationStep = (stepId: string) => {
  const navigationSteps = ['step-1-programs', 'step-1-periods', 'step-1-study-plans', 'step-1-subjects', 'step-1-groups', 'step-2-students', 'step-3-enrollments', 'step-4-dashboard']
  return navigationSteps.includes(stepId)
}

const isFormNavigationStep = (stepId: string) => {
  const formNavigationSteps = ['programs-new-button', 'periods-new-button', 'study-plans-new-button', 'subjects-new-button', 'groups-new-button', 'students-new-button', 'enrollments-new-button']
