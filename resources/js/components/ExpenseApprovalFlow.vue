...
const requiresCouncilApproval = computed(() => {
  return props.councilApprovalRequired && 
         props.expense.total_amount >= props.approvalThreshold
})
  ...
  switch (status) {
    ...
    case 'aprobado':
      return {
        ...
        description: needsCouncil ? 'El gasto ha sido aprobado por el administrador y el consejo.' : 'El gasto ha sido aprobado por el administrador.',
        icon: '✅',
        color: 'text-blue-600'
      }
    case 'pagado':
...
<template>
  <div class="space-y-6">
    ...
    <div :class="[
      ...
      expense.status === 'borrador' ? 'bg-gray-50 border-gray-300' :
      expense.status === 'pendiente' ? 'bg-yellow-50 border-yellow-300' :
      expense.status === 'pendiente_concejo' ? 'bg-orange-50 border-orange-300' :
      expense.status === 'aprobado' ? 'bg-blue-50 border-blue-300' :
      expense.status === 'pagado' ? 'bg-green-50 border-green-300' :
      expense.status === 'rechazado' ? 'bg-red-50 border-red-300' :
      expense.status === 'cancelado' ? 'bg-gray-50 border-gray-400' :
      'bg-white border-gray-200'
    ]">
      <!-- Animated Background Indicator -->
      <div v-if="['pendiente', 'pendiente_concejo'].includes(expense.status)" 
      ...
      <div class="flex items-start space-x-4 relative z-10">
        <div :class="[
          ...
          expense.status === 'borrador' ? 'bg-gray-100 text-gray-700' :
          expense.status === 'pendiente' ? 'bg-yellow-100 text-yellow-700 animate-bounce' :
          expense.status === 'pendiente_concejo' ? 'bg-orange-100 text-orange-700 animate-bounce' :
          expense.status === 'aprobado' ? 'bg-blue-100 text-blue-700' :
          expense.status === 'pagado' ? 'bg-green-100 text-green-700' :
          expense.status === 'rechazado' ? 'bg-red-100 text-red-700' :
          expense.status === 'cancelado' ? 'bg-gray-200 text-gray-600' :
          'bg-gray-100 text-gray-600'
        ]">
          {{ currentStepInfo.icon }}
        </div>
        <div class="flex-1">
          <h3 :class="['text-xl font-bold mb-2', currentStepInfo.color]">
          ...
          <div class="mb-3">
            ...
            <div class="w-full bg-gray-200 rounded-full h-2">
              <div :class="[
                ...
                expense.status === 'pagado' ? 'bg-green-500' :
                expense.status === 'aprobado' ? 'bg-blue-500' :
                expense.status === 'pendiente_concejo' ? 'bg-orange-500' :
                expense.status === 'pendiente' ? 'bg-yellow-500' :
                expense.status === 'borrador' ? 'bg-gray-400' :
                'bg-red-500'
              ]" :style="{ width: getProgressPercentage() + '%' }"></div>
            </div>
          </div>

          <div v-if="requiresCouncilApproval && ['borrador', 'pendiente'].includes(expense.status)" 
               class="p-3 bg-orange-100 border-l-4 border-orange-500 rounded text-sm text-orange-800">
      ...
      <div class="space-y-4">
        ...
        >
          ...
          <div class="flex-1 min-w-0">
            ...
            <p v-if="item.date" class="text-xs text-gray-500 mt-1">
