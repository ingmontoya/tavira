<template>
    <form @submit.prevent="handleSubmit" :class="formClasses">
        ...
        <!-- Rate Limit Warning -->
        <SecurityAlert v-if="rateLimitWarning" type="warning" :message="rateLimitWarning" :dismissible="false" class="mb-4" />

        <!-- Security Errors -->
        <SecurityAlert v-for="error in securityErrors" :key="error" type="error" :message="error" :dismissible="true" class="mb-4" />
    </form>
</template>
