...
const breadcrumbItems = computed(() => [
    ...
    {
        title: isEditing.value ? 'Editar estudiante' : 'Nuevo estudiante',
        href: isEditing.value ? `/students/${props.student?.id}/edit` : '/students/create',
    },
]);
...
const form = useForm({
    ...
    credits_completed: props.student?.credits_completed || 0,
    total_credits: props.student?.total_credits || 0,
    progress_rate: props.student?.progress_rate || 0,
...
<template>
    <AppLayout :breadcrumbs="breadcrumbItems">
        <div class="flex h-full w-full max-w-4xl flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
            ...
            <Head :title="isEditing ? 'Editar estudiante' : 'Agregar estudiante'" />
            <HeadingSmall :title="isEditing ? 'Editar estudiante' : 'Agregar estudiante'"
                :description="isEditing ? 'Modifica la información del estudiante' : 'Crea un nuevo estudiante'" />
            ...
            <form @submit.prevent="submit" class="space-y-6">
                ...
                <div class="flex items-center gap-4">
                    <Button type="submit" :disabled="form.processing"> {{ isEditing ? 'Actualizar' : 'Crear' }}
                        Estudiante </Button>
                    ...
                    <Transition enter-active-class="transition ease-in-out" enter-from-class="opacity-0"
                        ...
                        <p v-show="form.recentlySuccessful" class="text-sm text-neutral-600">
                            {{ isEditing ? 'Actualizado' : 'Creado' }} exitosamente.
                        </p>
                    </Transition>
                </div>
            </form>
