...
import { ArrowLeft, CheckCircle, Clock, Eye, Pause, Play, Plus, Vote, X } from 'lucide-vue-next';
        ...
        <div class="container mx-auto max-w-7xl px-4 py-8">
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription> Gestiona todas las votaciones de esta asamblea </CardDescription>
                </CardHeader>
                <CardContent>
                    <div v-if="votes.data.length > 0" class="space-y-4">
                                    ...
                                    <TableRow v-for="vote in votes.data" :key="vote.id">
                                        ...
                                        <TableCell>
                                            <div>
                                                ...
                                                <p v-if="vote.description" class="mt-1 text-sm text-gray-500">
                                        ...
                                        <TableCell>
                                            <div class="flex items-center gap-2">
                                                ...
                                                <span class="text-xs text-gray-500"> {{ vote.required_percentage }}% </span>
                                            </div>
                                        </TableCell>
                                        ...
                                        <TableCell>
                                            <div class="space-y-2">
                                                <div class="text-sm">{{ vote.total_votes }} votos</div>
                                        ...
                                        <TableCell>
                                            <div v-if="vote.results" class="space-y-1">
                                                ...
                                                <div class="text-xs text-gray-500">{{ vote.results.percentage.toFixed(1) }}% de aprobación</div>
                                            </div>
                                        ...
                                        <TableCell>
                                            <div class="flex items-center gap-1">
                                                ...
                                                <template v-if="vote.status === 'draft'">
                                                    <Button @click="activateVote(vote.id)" size="sm" variant="ghost" title="Activar votación">
                                                        <Play class="h-4 w-4" />
                                                    </Button>
                                                ...
                                                <template v-else-if="vote.status === 'active'">
                                                    <Button @click="closeVote(vote.id)" size="sm" variant="ghost" title="Cerrar votación">
                                                        <Pause class="h-4 w-4" />
                                                    </Button>
                                                </template>
                                            </div>
                        ...
                        <!-- Pagination -->
                        <div v-if="votes.total > 0" class="flex items-center justify-between border-t pt-6">
                            <p class="text-sm text-gray-700">
                                Mostrando <span class="font-medium">{{ votes.from }}</span> a <span class="font-medium">{{ votes.to }}</span> de
                                <span class="font-medium">{{ votes.total }}</span> resultados
                            </p>
                            <div class="flex space-x-2">
                                <Button v-if="votes.prev_page_url" @click="router.visit(votes.prev_page_url!)" variant="outline" size="sm">
                                    Anterior
                                </Button>
                                <Button v-if="votes.next_page_url" @click="router.visit(votes.next_page_url!)" variant="outline" size="sm">
                                    Siguiente
                                </Button>
                            </div>
                        </div>
                    ...
                    <div v-else class="py-12 text-center">
                        <Vote class="mx-auto mb-4 h-12 w-12 text-gray-400" />
                        <h3 class="mb-2 text-lg font-medium text-gray-900">No hay votaciones</h3>
                        <p class="mb-4 text-gray-500">Crea la primera votación para esta asamblea.</p>
                        <Link :href="`/assemblies/${assembly.id}/votes/create`" v-if="assembly.status === 'in_progress'">
                            <Button>
