...
const cancelExpense = () => {
    ...
    if (reason !== null) {
        // Allow empty string but not null (user cancelled)
                ...
                <div class="flex items-center justify-between">
                    <div class="space-y-1">
                        <div class="flex items-center gap-3">
                            ...
                            <AlertTriangle v-if="expense.is_overdue" class="h-5 w-5 text-red-500" :title="`Vencido ${expense.days_overdue} días`" />
                        </div>
                ...
                <Tabs default-value="details" class="space-y-4">
                        ...
                        <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
                            ...
                            <div class="space-y-4 lg:col-span-2">
                                ...
                                <Card>
                                    ...
                                    <CardContent class="space-y-4">
                                        <div class="grid grid-cols-2 gap-4">
                                            <div>
                                                <h4 class="mb-1 text-sm font-medium text-muted-foreground">Número</h4>
                                                <p class="font-medium">{{ expense.expense_number }}</p>
                                            </div>
                                            <div>
                                                <h4 class="mb-1 text-sm font-medium text-muted-foreground">Categoría</h4>
                                        ...
                                        <div>
                                            <h4 class="mb-1 text-sm font-medium text-muted-foreground">Descripción</h4>
                                            <p>{{ expense.description }}</p>
                                        </div>

                                        <div class="grid grid-cols-2 gap-4">
                                            <div>
                                                <h4 class="mb-1 text-sm font-medium text-muted-foreground">Fecha del Gasto</h4>
                                                <p>{{ new Date(expense.expense_date).toLocaleDateString('es-CO') }}</p>
                                            </div>
                                            <div v-if="expense.due_date">
                                                <h4 class="mb-1 text-sm font-medium text-muted-foreground">Fecha de Vencimiento</h4>
                                                <p class="flex items-center gap-2">
                                                    {{ new Date(expense.due_date).toLocaleDateString('es-CO') }}
                                                    <AlertTriangle v-if="expense.is_overdue" class="h-4 w-4 text-red-500" />
                                                </p>
                                            </div>
                                        </div>
                                        ...
                                        <div v-if="expense.notes" class="mt-4">
                                            ...
                                            <p class="rounded-md bg-muted p-3 text-sm whitespace-pre-wrap">{{ expense.notes }}</p>
                                        </div>
                                    </CardContent>
                                </Card>
                                ...
                                <Card>
                                    ...
                                    <CardContent class="space-y-2">
                                        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                                            <div>
                                                <h4 class="mb-1 text-sm font-medium text-muted-foreground">Nombre</h4>
                                                <p class="font-medium">{{ expense.vendor_name }}</p>
                                            </div>
                                            <div v-if="expense.vendor_document">
                                                <h4 class="mb-1 text-sm font-medium text-muted-foreground">Documento</h4>
                                                <p>{{ expense.vendor_document }}</p>
                                            </div>
                                            <div v-if="expense.vendor_email">
                                                <h4 class="mb-1 text-sm font-medium text-muted-foreground">Email</h4>
                                                <p>
                                            ...
                                            <div v-if="expense.vendor_phone">
                                                <h4 class="mb-1 text-sm font-medium text-muted-foreground">Teléfono</h4>
                                                <p>
                                ...
                                <Card v-if="expense.status === 'pagado'">
                                    ...
                                    <CardContent class="space-y-2">
                                        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                                            <div>
                                                <h4 class="mb-1 text-sm font-medium text-muted-foreground">Fecha de Pago</h4>
                                                <p class="font-medium">
                                                    {{ expense.paid_at ? new Date(expense.paid_at).toLocaleDateString('es-CO') : 'N/A' }}
                                                </p>
                                            </div>
                                            <div v-if="expense.payment_method">
                                                <h4 class="mb-1 text-sm font-medium text-muted-foreground">Método de Pago</h4>
                                                <p>{{ expense.payment_method }}</p>
                                            </div>
                                            <div v-if="expense.payment_reference" class="col-span-2">
                                                <h4 class="mb-1 text-sm font-medium text-muted-foreground">Referencia</h4>
                            ...
                            <div class="space-y-4">
                                ...
                                <Card>
                                    ...
                                    <CardContent class="space-y-3">
                                        ...
                                        <div class="space-y-2 text-sm">
                                            ...
                                            <div v-if="expense.approved_at" class="ml-5 flex items-center gap-2">
                                ...
                                <Card v-if="expense.debit_account && expense.credit_account">
                                    ...
                                    <CardContent class="space-y-3">
                                        <div>
                                            <h5 class="mb-1 text-xs font-medium text-muted-foreground">Cuenta a Debitar</h5>
                                            <p class="text-sm">{{ expense.debit_account.full_name }}</p>
                                        </div>
                                        <div>
                                            <h5 class="mb-1 text-xs font-medium text-muted-foreground">Cuenta a Acreditar</h5>
                    ...
                    <TabsContent value="accounting" class="space-y-4">
                        <Card>
                            ...
                            <CardContent>
                                <div v-if="expense.accounting_transactions.length === 0" class="py-8 text-center text-muted-foreground">
                                    No hay transacciones contables asociadas
                                </div>
                                <div v-else class="space-y-4">
                                    <div v-for="transaction in expense.accounting_transactions" :key="transaction.id" class="rounded-lg border p-4">
                                        ...
                                        <p class="mb-3 text-sm text-muted-foreground">{{ transaction.description }}</p>
                                        ...
                                        <div class="mt-3 flex items-center justify-between border-t pt-3 text-sm font-medium">
                                            <span>Totales:</span>
                                ...
                                <div class="space-y-4">
                                    ...
                                    <div v-if="expense.paid_at" class="flex items-start gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-xs text-muted-foreground">El {{ new Date(expense.paid_at).toLocaleString('es-CO') }}</p>
                                        </div>
                                    </div>
                                </div>
                            </CardContent>
                ...
                <DialogContent class="sm:max-w-[425px]">
                    <form @submit.prevent="markAsPaid">
                        <DialogHeader>
                            ...
                            <DialogDescription> Registre la información del pago para el gasto {{ expense.expense_number }} </DialogDescription>
                        </DialogHeader>
                        ...
                        <DialogFooter>
                            <Button type="button" variant="outline" @click="isMarkAsPaidDialogOpen = false" :disabled="markAsPaidForm.processing">
                                Cancelar
                            </Button>
                            <Button type="submit" :disabled="markAsPaidForm.processing">
                                <CreditCard class="mr-2 h-4 w-4" />
