...
import { AlertCircle, Check, Download, Eye, FileSpreadsheet, Filter, Upload, X } from 'lucide-vue-next';
...
<template>
    <AppLayout>
        ...
        <div class="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
            ...
            <!-- Batch Actions -->
            <div v-if="selectedImports.length > 0" class="flex items-center gap-2 rounded-lg border bg-blue-50 p-4">
                <span class="text-sm text-blue-700">{{ selectedImports.length }} registros seleccionados</span>
                <div class="ml-auto flex gap-2">
                    <Button @click="processBatch('create_payments')" size="sm" variant="outline"> Crear Pagos </Button>
                    <Button @click="processBatch('reject_all')" size="sm" variant="destructive"> Rechazar Todos </Button>
                </div>
            </div>
                    ...
                    <Table>
                        <TableHeader>
                            <TableRow>
                                <TableHead class="w-12">
                                    <Checkbox :checked="allSelected" :indeterminate="someSelected" @update:checked="toggleAll" />
                                </TableHead>
                        ...
                        <TableBody>
                            <TableRow v-for="import_item in imports?.data" :key="import_item.id">
                                ...
                                <TableCell>
                                    <Badge
                                        :class="import_item?.status_badge?.class"
                                        class="flex w-fit items-center gap-1"
                                        v-if="import_item?.status_badge"
                                    >
                                ...
                                <TableCell>
                                    <div class="flex items-center gap-2">
                                        ...
                                        <Button v-if="import_item?.payment?.id" asChild size="sm" variant="outline">
                                            <Link :href="route('finance.payments.show', import_item.payment.id)"> Ver Pago </Link>
                                        </Button>
                                    </div>
                                </TableCell>
                            </TableRow>
                    ...
                    <div v-if="imports?.links?.length > 3" class="mt-4 flex justify-center">
                        <nav class="flex items-center gap-1">
                            <template v-for="link in imports.links" :key="link.label">
                                ...
                                <span
                                    v-else
                                    :class="['rounded border px-3 py-2 text-sm', 'cursor-not-allowed border-gray-200 bg-gray-100 text-gray-400']"
                                    v-html="link.label"
                                />
                            </template>
                        </nav>
        ...
        <Dialog v-model:open="showUploadDialog">
            <DialogContent>
                <DialogHeader>
                    ...
                    <DialogDescription> Selecciona el archivo Excel (.xlsx) exportado desde la plataforma Jelpit </DialogDescription>
                </DialogHeader>

                <form @submit.prevent="uploadFile" class="space-y-4">
                    <div class="space-y-2">
                        <Label for="file">Archivo Excel</Label>
                        <Input id="file" type="file" accept=".xlsx,.xls" @change="uploadForm.file = $event.target.files[0]" required />
                        <p class="text-sm text-gray-500">Formatos soportados: .xlsx, .xls (máximo 10MB)</p>
                    ...
                    <div class="flex justify-end gap-2">
                        <Button @click="showUploadDialog = false" variant="outline" type="button"> Cancelar </Button>
        ...
        <Dialog v-model:open="showBatchDialog">
            <DialogContent>
                ...
                <form @submit.prevent="submitBatch" class="space-y-4">
                    ...
                    <div class="flex justify-end gap-2">
                        <Button @click="showBatchDialog = false" variant="outline" type="button"> Cancelar </Button>
                        <Button
                            type="submit"
