...
import { ArrowLeft, Calendar, Check, CheckCircle, Clock, DollarSign, Edit, FileText, MapPin, Play, User, Wrench } from 'lucide-vue-next';
...
const canApprove = computed(() => {
    return ['budgeted', 'pending_approval'].includes(props.maintenanceRequest.status);
});

const canAssign = computed(() => {
    return props.maintenanceRequest.status === 'approved';
});

const canStartWork = computed(() => {
    return props.maintenanceRequest.status === 'assigned';
});

const canComplete = computed(() => {
    return props.maintenanceRequest.status === 'in_progress';
});
