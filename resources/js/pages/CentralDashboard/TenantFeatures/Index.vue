        ...
        <div class="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
                ...
                <Card v-for="tenant in filteredTenants" :key="tenant.id" class="p-6">
                    ...
                    <div class="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4">
                        <div v-for="feature in availableFeatures" :key="feature" class="flex items-center space-x-2">
                            ...
                            />
                            <Label :for="`${tenant.id}-${feature}`" class="cursor-pointer text-sm font-normal">
                                {{ getFeatureLabel(feature) }}
                            </Label>
                        </div>
                    </div>
                    ...
                    <div class="mt-6 flex items-center justify-between border-t pt-4">
                        ...
                        <div class="flex space-x-1">
                            <span
                                ...
                                :key="feature"
                                :class="['h-2 w-2 rounded-full transition-colors', isFeatureEnabled(tenant, feature) ? 'bg-green-500' : 'bg-muted']"
                            ></span>
                        </div>
                    </div>
                </Card>
            ...
            <div v-if="tenants.links && tenants.links.length > 0" class="flex items-center justify-end space-x-2 py-4">
                <div class="flex-1 text-sm text-muted-foreground">
                    Mostrando {{ tenants.from || 1 }} a {{ tenants.to || tenants.data.length }} de
                    {{ tenants.total || tenants.data.length }} resultados
                </div>
                <div class="space-x-2">
                    <Button variant="outline" size="sm" :disabled="!tenants.prev_page_url" @click="navigateToPage(tenants.prev_page_url)">
                        Anterior
                    </Button>
                    <Button variant="outline" size="sm" :disabled="!tenants.next_page_url" @click="navigateToPage(tenants.next_page_url)">
                        Siguiente
                    </Button>
                </div>
            </div>
...
function applyTemplate(tenantId: string, template: string) {
    ...
    router.post(
        route('tenant-features.apply-template', tenantId),
        {
            template,
        },
        {
...
const filteredTenants = computed(() => {
    ...
    if (specificFeatureFilter.value !== 'all') {
        filtered = filtered.filter((tenant) => isFeatureEnabled(tenant, specificFeatureFilter.value));
    }
...
const hasActiveFilters = computed(() => {
    return searchQuery.value !== '' || templateFilter.value !== 'all' || featuresCountFilter.value !== 'all' || specificFeatureFilter.value !== 'all';
});
